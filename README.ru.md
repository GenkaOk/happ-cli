...
| `--no-routes`    | `false`      | создать TUN без правки маршрутов (режим tun)               |
| `--require-check` | `false`      | завершиться с ошибкой, если проверка подключения не прошла |
...
5. флаг `--no-routes` пропускает правку маршрутов (только создаёт TUN),
...
- **Hysteria2 / TUIC / WireGuard** xray-core не умеет (парсятся и показываются как
  ...
  полностью.
- **TUN и `--system-proxy` пока только для macOS**. Режим TUN для Linux
  реализован, но ещё не протестирован на реальном железе.
...
| `internal/tunnel`   | режим TUN: tun2socks + управление маршрутами (macOS + Linux) |
